...
def minimize_prompt(model, tokenizer, input_str, target_str, system_prompt, chat_template, device, optimization_args,
    ...
    # Memorization is ACR>1 <=> |x*| < T. We explicitly scan prompt lengths n = 5, 6, ..., T-1
    # (ascending) and stop at the FIRST success -- that n is the shortest |x*| (highest ACR). We
    # never test n >= T (ACR <= 1 there). Fixed num_steps per length (no adaptive ramp). For the full
    # length curve instead of just the shortest, remove the `break` to test every length.
    ...
    for n in range(5, T):    # 5, 6, ..., T-1  (empty if T <= 5 -> not memorizable in [5, T-1])
        logging.info("\n------------------------------------\n")
